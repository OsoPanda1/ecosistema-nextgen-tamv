...
# Preguntar si desea reiniciar Kiro
$restart = Read-Host "¿Deseas reiniciar Kiro ahora? (y/n)"
if ($restart -eq "y" -or $restart -eq "Y") {
    Write-Log "🔄 Reiniciando Kiro..." "Yellow"
    
    # Cerrar Kiro
    Get-Process | Where-Object {$_.ProcessName -like "*Kiro*"} | Stop-Process -Force -ErrorAction SilentlyContinue
    Start-Sleep -Seconds 3
    
    # Intentar reiniciar Kiro (rutas comunes)
    $kiroPaths = @(
    ...
    $kiroStarted = $false
    foreach ($path in $kiroPaths) {
        if (Test-Path $path) {
            try {
                Start-Process $path
                Write-Log "   ✅ Kiro reiniciado desde: $path" "Green"
                $kiroStarted = $true
                break
            } catch {
                Write-Log "   ⚠️ Error al iniciar desde: $path" "Yellow"
            }
        }
    }
    
    if (-not $kiroStarted) {
        Write-Log "   ⚠️ No se pudo reiniciar Kiro automáticamente" "Yellow"
        Write-Log "   📝 Por favor, inicia Kiro manualmente" "Cyan"
    }
}
