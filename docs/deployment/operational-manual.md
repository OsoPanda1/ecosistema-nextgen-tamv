...
## 10) Auditoría técnica (hallazgos y correcciones)

### Hallazgos críticos
- **Control de acceso insuficiente** en listados y operaciones sensibles (economía, BookPI, EOCT, Isabella, DreamSpaces).
- **Pérdida de trazabilidad** potencial por cascadas en auditorías históricas.
- **XR Gateway sin autenticación previa**.

### Correcciones aplicadas
- **Autenticación obligatoria** y **scoping por actor** en listados sensibles.
- **Permisos explícitos** en DreamSpaces (owner/guardian/admin).
- **Persistencia de memoria** con `ON DELETE SET NULL` en tablas de auditoría.
- **XR Gateway autenticado** por JWT.
- **Validaciones de montos** en economía y prevención de auto-escalación.

### Próximos pasos
- Automatizar pruebas de seguridad (OWASP, SAST/DAST).
- Añadir pruebas unitarias por módulo (protocolos, guardianía, economía, identidad).
- Instrumentar métricas y alertas por dominio.
